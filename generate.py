...
def wa_link(message):
    """Build a WhatsApp link with a prefilled order message naming the specific service/product."""
...
def breadcrumb_schema(items_full):
    entries = []
    for i, (name, url) in enumerate(items_full, start=1):
        entries.append('{"@type":"ListItem","position":%d,"name":"%s","item":"%s"}' % (i, name, url))
    return '{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[' + ",".join(entries) + "]}"
